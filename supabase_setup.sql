...
-- Read-only session verification checking token validity and expiration without database writes
CREATE OR REPLACE FUNCTION public.verify_session(p_email TEXT, p_token UUID)
RETURNS TABLE (
    id BIGINT,
    name TEXT,
    email TEXT,
...
-- Secure subscription update requiring a valid session token (privileged edge execution only)
...
CREATE OR REPLACE FUNCTION public.upgrade_plan_after_payment(p_email TEXT, p_plan TEXT)
RETURNS VOID AS $$
BEGIN
    -- Validate parameters
    IF p_email IS NULL OR p_email !~* '^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$' THEN
...
-- Revoke execution rights from public, anon, and authenticated roles to block direct client execution
REVOKE EXECUTE ON FUNCTION public.update_subscription_secure(TEXT, UUID, TEXT) FROM public, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.upgrade_plan_after_payment(TEXT, TEXT) FROM public, anon, authenticated;
